...
class App extends Component {
  constructor(){
    super();
    this.state={nama:'',usia:'',alamat:'',pegawai:[]}
  }
  klik(){
    this.setState({
  ...
  klik2(){
    ...
    axios.post('http://localhost:3000/api/data-pegawais',{
      nama : x,
      usia : y,
      alamat : z
    })
  ...
  klik3(){
    axios.get('http://localhost:3000/api/data-pegawais')
    .then((ambilData)=>{
  ...
  render() {
    const data = this.state.pegawai.map((item, index)=>{
      ...
      return <tr key={index}><td>{id}</td><td>{nama}</td><td>{usia}</td><td>{alamat}</td></tr>;
    })
    return (
      <div>
      <center>
